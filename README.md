<h1 align="center">Hey, I'm Albert 👋</h1>
<h3 align="center">Network & Full-Stack Developer · Cisco-Certified · Serial Systems Builder</h3>

<p align="center">
  🎓 Studying IT, open to opportunities &nbsp;·&nbsp; 📍 Philippines &nbsp;·&nbsp; ☕ Powered by coffee and PHP
</p>

---

### 🧭 About me

I'm the person who says "I'll just build a quick system for that" and then disappears for two weeks into RFID scanners, MySQL schemas, and admin dashboards. I split my time between two worlds that don't usually mix: **routing packets** and **routing HTTP requests**. Cisco-certified on one side, PHP/MySQL on the other — I like knowing how the whole stack works, from the switch to the `<div>`.

Currently studying **BS Information Technology** at Eastern Samar State University – Guiuan Campus, and building real systems for my College of Computer Studies along the way (because tutorials are fine, but production bugs teach faster).

- 🔭 Recently shipped: RFID attendance systems, fines monitoring, and campus e-logbooks
- 🌱 Currently leveling up: cybersecurity + backend architecture
- 💬 Ask me about: Cisco networking, PHP/MySQL systems, or why RFID debouncing is harder than it sounds
- 📫 Reach me: **aceabris00@gmail.com**

---

### 🛠 Stack

| Area | Tools |
|---|---|
| 🌐 Networking | Cisco IOS, Routing & Switching, Wireshark, Network Security |
| 🖥 Backend | PHP, MySQL, SQL |
| 🎨 Frontend | HTML, CSS, JavaScript |
| 🧩 Programming | C++, C#, Java |

---

### 🏅 Certifications

<p align="left">
  <a href="https://www.credly.com/badges/edb0f3e0-d69d-4e36-bb30-b12ed230dd69" target="_blank">
    <img src="https://images.credly.com/size/220x220/images/networking-academy-badge-placeholder/image.png" alt="Network Addressing and Basic Troubleshooting" height="90" />
  </a>
  &nbsp;&nbsp;
  <a href="https://www.credly.com/badges/e68d2fc9-40ee-4e05-b3c8-89dd0510d142" target="_blank">
    <img src="https://images.credly.com/size/220x220/images/cyber-threat-management-badge-placeholder/image.png" alt="Cyber Threat Management" height="90" />
  </a>
</p>

- **Network Addressing and Basic Troubleshooting** — Cisco Networking Academy · May 2026 → [Verify](https://www.credly.com/badges/edb0f3e0-d69d-4e36-bb30-b12ed230dd69)
- **Cyber Threat Management** — Cisco Networking Academy · May 2024 → [Verify](https://www.credly.com/badges/e68d2fc9-40ee-4e05-b3c8-89dd0510d142)

*(Badge images above are placeholders — swap in the real PNGs from `assets/certifications/` or your Credly badge embed links.)*

---

### 🚀 Featured builds

**[RFID Based Computer Laboratory E-Logbook System](https://github.com/aceabris/labbook)**
An RFID-based e-logbook for computer labs — students tap in, hours get logged, nobody argues about attendance.
`HTML` `CSS` `JavaScript` `PHP` `MySQL` — [Live demo ↗](https://essu-scan.free.nf/labbook/information.php)

**[ITSO Events RFID Attendance & Fines Monitoring System](https://github.com/aceabris/itso-events)**
RFID attendance tracking wired straight into fines monitoring for ITSO events — one tap, two problems solved.
`HTML` `CSS` `JavaScript` `PHP` `MySQL` — [Live demo ↗](https://essu-scan.free.nf/itso-events/scanner.php)

*(GitHub links above are placeholders — replace `aceabris/labbook` and `aceabris/itso-events` with your real repo URLs.)*

---

### 📊 GitHub stats

<p align="left">
  <img src="https://github-readme-stats.vercel.app/api?username=aceabris&show_icons=true&theme=tokyonight&hide_border=true" alt="Albert's GitHub stats" height="165" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=aceabris&layout=compact&theme=tokyonight&hide_border=true" alt="Top languages" height="165" />
</p>

<p align="left">
  <img src="https://streak-stats.demolab.com/?user=aceabris&theme=tokyonight&hide_border=true" alt="GitHub streak" height="165" />
</p>

---

### 🔗 Find me elsewhere

<p align="left">
  <a href="mailto:aceabris00@gmail.com">✉️ Email</a> &nbsp;·&nbsp;
  <a href="https://linkedin.com/" target="_blank">💼 LinkedIn</a> &nbsp;·&nbsp;
  <a href="https://twitter.com/" target="_blank">🐦 Twitter</a> &nbsp;·&nbsp;
  <a href="https://www.credly.com/users/john-albert-abris" target="_blank">🏅 Credly</a>
</p>

<p align="center"><sub>Thanks for scrolling this far — go check out one of the demos above, they actually work. 🙂</sub></p>
